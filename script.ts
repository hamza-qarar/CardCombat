...
// Union-Type: nur diese vier Strings sind als Auswahl erlaubt
type Choice = 'König' | 'Dame' | 'Bube' | 'Ass';

// Alle möglichen Optionen – wird für die zufällige Gegnerwahl genutzt
const options: Choice[] = ['König', 'Dame', 'Bube', 'Ass'];

// Beschreibt das visuelle Aussehen einer Karte
interface CardInfo {
    rank: string;       // Buchstabe oben links / unten rechts (K, Q, J, A)
    suit: string;       // Kartenzeichen (♠ ♥ ♣ ♦)
    colorClass: string; // CSS-Klasse: 'black' oder 'red'
}
...
function play(selection: Choice): void {
    ...
    (document.getElementById('wins')   as HTMLElement).textContent = String(winCount);
    (document.getElementById('losses') as HTMLElement).textContent = String(lossCount);
    (document.getElementById('draws')  as HTMLElement).textContent = String(drawCount);
}
